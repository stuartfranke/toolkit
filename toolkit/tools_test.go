...
func TestTools_PushJSONToRemote(t *testing.T) {
	...
	_, _, err := testTools.PushJSONToRemote("http://domain.tld", foo, client)
	if err != nil {
...
func TestTools_RandomString(t *testing.T) {
	...
	if len(s) != 10 {
		t.Error("incorrect length random string returned")
	}
}
...
var uploadTests = []struct {
	...
	{name: "allowed rename", allowedTypes: []string{"image/jpeg", "image/png"}, renameFile: true,
		errorExpected: false},
	{name: "not allowed", allowedTypes: []string{"image/jpeg"}, renameFile: false,
		errorExpected: true},
}
...
func TestTools_DownloadStaticFile(t *testing.T) {
	...
	var testTool Tools
	testTool.DownloadStaticFile(rr, req, dir, file, "document.pdf")
	...
	downloadSize, err := strconv.Atoi(res.Header["Content-Length"][0])
	if err != nil {
		t.Error(err)
	}

	if int64(downloadSize) != fileInfo.Size() {
		t.Error("wrong content length of", res.Header["Content-Length"])
	}

	if res.Header["Content-Disposition"][0] != "attachment; filename=\"document.pdf\"" {
